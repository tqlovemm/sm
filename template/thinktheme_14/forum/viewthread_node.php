...
<tr>
	<td class="pls" rowspan="2">
		...
		<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
			<!--{if $_G['setting']['authoronleft']}-->
				<div class="pi" style="width:180px;">
		...
		<!--{if $post['authorid'] && $post['username'] && !$post['anonymous']}-->
			...
			<div class="tns">
				<table cellspacing="0" cellpadding="0">
					<th><p><a href="home.php?mod=space&uid=$post[authorid]&do=thread&view=me&from=space" class=""><!--{echo dnumber($post[threads])}--></a></p>{lang threads}</th>
					<th>
						<!--{if helper_access::check_module('follow')}-->
						<p><a href="home.php?mod=follow&do=follower&uid=$post[authorid]" class=""><!--{echo dnumber($post[follower])}--></a></p>{lang follower}
						<!--{else}-->
						<p><a href="home.php?mod=space&do=follower&uid=$post[authorid]" class=""><!--{echo dnumber($post[friends])}--></a></p>粉丝
						<!--{/if}-->
					</th>
					<td><p><a href="home.php?mod=space&uid=$post[authorid]&do=profile" class=""><!--{echo dnumber($post[extcredits2])}--></a></p>金钱</td>
				</table>
			</div>
			...
			<!--{if $post[authorid] != $_G[uid]}-->
				<ul class="xl o cl tt_postlist_btn">
					<!--{if helper_access::check_module('follow')}-->
					<!--<li class="addflw z">
						<a href="home.php?mod=spacecp&ac=follow&op=add&hash={FORMHASH}&fuid=$post[authorid]" id="followmod_$post[authorid]" title="{lang follow}" class="" onclick="showWindow('followmod', this.href, 'get', 0);">加关注</a>
					</li>-->
					<!--{/if}-->
					<li class="addflw z"><a href="javascri pt:;" onclick="addfollow($post[authorid],this);"  class="">加关注</a></li>
					<li class="pm2 y"><a href="home.php?mod=spacecp&ac=pm&op=showmsg&handlekey=showmsg_$post[authorid]&touid=$post[authorid]&pmid=0&daterange=2&pid=$post[pid]&tid=$post[tid]" onclick="showWindow('sendpm', this.href);" title="{lang viewthread_left_sendpm}" class="">{lang viewthread_left_sendpm}</a></li>
				</ul>
			<!--{/if}-->
	...
	<td class="plc" width="799">
		<div class="pi tt_postlist_c_right_top">
			<!--{if !IS_ROBOT}-->
			...
			<div class="pti">
				...
				<!--{if $post['authorid'] && !$post['anonymous']}-->
					...
					<em id="authorposton$post[pid]">{lang poston} {eval echo date('Y-m-d H:i:s',$post['dbdateline'])}</em>
					<!--{if $post['status'] & 8}-->
		...
		<div class="pct tt_postlist_c_right_center">
		...
		<!--{if helper_access::check_module('collection') && !$_G['forum']['disablecollect']}-->
			<!--{if $post['relatecollection']}-->
				<div class="cm" style="padding-left:20px">
	...
	<tr><td class="plc plm pct">
		...
		<!--{if !IS_ROBOT && $post['first'] && !$_G['forum_thread']['archiveid']}-->
			...
			<!--{if $post['invisible'] == 0}-->
				...
				<!-- [[[tiezi_caozuo -->
				<div class="tt_tiezi_caozuo">
					<a href="javascript:;" id="k_favorite" onclick="likethread($_G[tid])"><i>{lang thread_favorite}<span id="favoritenumber">({$_G['forum_thread']['favtimes']})</span></i></a>
					<a href="javascript:;" id="ak_rate" onclick="showWindow('rate', 'forum.php?mod=misc&action=rate&tid=$_G[tid]&pid=$post[pid]', 'get', -1);return false;" title="{echo count($postlist[$post[pid]][totalrate]);} {lang people_score}" style="background-position:-85px top;">评分(<!--{if $post['ratelogextcredits'] != ''}-->
						<!--{loop $post['ratelogextcredits'] $id $score}-->$score
						<!--{/loop}-->
					<!--{else}-->0<!--{/if}-->
					)</a>
					<a href="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&reppost=$post[pid]&extra=$_GET[extra]&page=$page" onclick="showWindow('reply', this.href)" style="background-position:-170px top;">回帖($_G[forum_thread][replies])</a>
				</div>
				<!-- tiezi_caozuo]]] -->
			...
			<!-- [[[tt_tiezi_more -->
			<div class="tt_tiezi_more cl">
				<!--{if $post['authoritem'] != ''}-->
				<dl class="z">
					<dt>楼主的帖子</dt>
						<dd>
							<ul>
								<!--{loop $post['authoritem'] $item}-->
								<li><a href="forum.php?mod=viewthread&tid=$item[tid]" target="_blank">{$item['subject']}</a></li>
								<!--{/loop}-->
								</ul>
						</dd>
				</dl>
...
<!--{else}-->
	...
	<tbody>
		...
		<tr class="ad"><td class=""></td><td class=""></td></tr>
	</tbody>
</table>
<!--{/if}-->
...
<!--{if '隐藏该处'=='' && $post['first']}-->
<!--{if $post['relateitem']}-->		
   <dl class="tt_like_box cl">
    <dt><img src="$_G['style']['styleimgdir']/like_title.png" /></dt>
    
    <!--{hook/viewthread_modaction}-->
    
    </dl>
	<style type="text/css">
	dl.tt_like_box{height:auto;padding:9px 10px 9px 188px;}
	div.rs_main{background:#fff;border:0;}
	</style>
<!--{/if}-->
<!--{/if}-->


<style>.ad .a_p {text-align:left;}</style>
