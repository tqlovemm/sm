...
<!--{else}-->
	...
	<div>
		...
		<!--{loop $sticklist $rpost}-->
			<div class="pstl xs1">
				<!--{if $rpost['authorid'] && !$rpost[anonymous]}-->
				<div class="psta"><a href="home.php?mod=space&uid=$rpost[authorid]" c="1">$rpost[avatar]</a></div>
				<div class="psti">
					<p>
						<a href="home.php?mod=space&uid=$rpost[authorid]" class="xi2 xw1">$rpost[author]</a> <a href="javascript:;" onclick="window.open('forum.php?mod=redirect&goto=findpost&ptid=$rpost[tid]&pid=$rpost[pid]')" class="xi2">{lang published_in_floor}</a>
						<!--{if $_G['group']['allowstickreply'] || $_G['forum_thread']['authorid'] == $_G['uid']}-->&nbsp;<a href="javascript:;" onclick="modaction('stickreply', $rpost[pid], '&undo=yes')" class="xi2">{lang admin_unstickreply}</a><!--{/if}-->
					</p>
				...
				<div class="psti">
					<p>
					<!--{if !$rpost['authorid']}-->
						<a href="javascript:;">$_G[setting][anonymoustext] <em>$rpost[useip]</em></a>
					<!--{elseif $rpost['authorid'] && $rpost['anonymous']}-->
						<!--{if $_G['forum']['ismoderator']}--><a href="home.php?mod=space&uid=$rpost[authorid]" target="_blank">$_G[setting][anonymoustext]</a><!--{else}-->$_G[setting][anonymoustext]<!--{/if}-->
					<!--{else}-->
						$rpost[author] <em>{lang member_deleted}</em>
					<!--{/if}-->
					 <a href="javascript:;" onclick="window.open('forum.php?mod=redirect&goto=findpost&ptid=$rpost[tid]&pid=$rpost[pid]')" class="xi2">{lang published_in_floor}</a>
					 <!--{if $_G['group']['allowstickreply'] || $_G['forum_thread']['authorid'] == $_G['uid']}-->&nbsp;<a href="javascript:;" onclick="modaction('stickreply', $rpost[pid], '&undo=yes')" class="xi2">{lang admin_unstickreply}</a><!--{/if}-->
					</p>
					<div class="mtn">$rpost[message]</div>
				</div>
			...
			<div class="pstl xs1">
				...
				<div class="psti">
					...
					<span class="xg1">
						...
						<!--{/if}-->
						<!--{if $_G['uid']!=$comment[authorid]}-->
	&nbsp;<a class="tt_huifu" href="forum.php?mod=misc&action=comment&tid=$post[tid]&pid=$post[pid]&extra=$_GET[extra]&page=$page{if $_G['forum_thread']['special'] == 127}&special=$specialextra{/if}" onclick="showWindow('comment', this.href, 'get', 0,'','$comment[author]')">»Ø¸´</a>
						<!--{/if}-->
					</span>
				</div>
			</div>
		...
		<dl id="ratelog_$post[pid]" class="rate{if !empty($_G['cookie']['ratecollapse'])} rate_collapse{/if}">
			...
			<!--{/if}-->
				...
				<!--{if $_G['setting']['ratelogon']}-->
					<table class="ratl">
						<tr>
							<th class="xw1" width="145"><b><a href="forum.php?mod=misc&action=viewratings&tid=$_G[tid]&pid=$post[pid]" onclick="showWindow('viewratings', this.href)" title="{lang rate_view}">{lang have} <span class="xi1"><!--{echo count($postlist[$post[pid]][totalrate]);}--></span> {lang people_score}</a></b></th>
							<!--{loop $post['ratelogextcredits'] $id $score}-->
							<th width="73"><i>{$_G['setting']['extcredits'][$id][title]}</i></th>
							<!--{/loop}-->
							<th>
								<a href="javascript:;" onclick="toggleRatelogCollapse('ratelog_$post[pid]', this);" class="y op"><!--{if !empty($_G['cookie']['ratecollapse'])}-->{lang open}<!--{else}-->{lang pack}<!--{/if}--></a>
								<i>{lang reason}</i>
							</th>
						</tr>
						<tbody class="ratl_l">
							...
							<tr id="rate_{$post[pid]}_{$uid}">
								...
								<!--{loop $post['ratelogextcredits'] $id $score}-->
									<!--{if $ratelog['score'][$id] > 0}-->
										<td class="xi1"><b> + $ratelog[score][$id]</b></td>
									<!--{else}-->
					...
					<p class="ratc">
						...
						<!--{loop $post['ratelogextcredits'] $id $score}-->
							<!--{if $score > 0}-->
								<span class="">{$_G['setting']['extcredits'][$id][title]} <b class="xi1">+ $score</b></span>&nbsp;
							<!--{else}-->
								<span class="xg1">{$_G['setting']['extcredits'][$id][title]} $score</span>&nbsp;
							<!--{/if}-->
						<!--{/loop}-->
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="forum.php?mod=misc&action=viewratings&tid=$_G[tid]&pid=$post[pid]" onclick="showWindow('viewratings', this.href)" title="{lang rate_view}" class="">{lang rate_view}</a>
					</p>
				<!--{else}-->
					<ul class="cl">
						<!--{loop $post['ratelog'] $uid $ratelog}-->
