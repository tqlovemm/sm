...
<div class="tt_mianbaoxie">您现在的位置：<a href="forum.php">$_G[setting][bbname]</a> $navigation$actionsubject <em>&rsaquo;</em> $actiontitle</div>
...
<div id="ct" class="ct2_a ct2_a_r wp cl">
		...
		<div class="bm bw0 cl"{if !$showthreadsorts && !$adveditor} id="editorbox"{/if}>
			...
			<div id="postbox">
				...
				<div class="mtm mbm pnpost">
					...
					<button type="submit" id="postsubmit" class="tt_fatie_btn" value="true" name="{if $_GET[action] == 'newthread'}topicsubmit{elseif $_GET[action] == 'reply'}replysubmit{elseif $_GET[action] == 'edit'}editsubmit{/if}">
					<!--{if $_GET[action] == 'newthread'}-->
					...
					<!--{elseif $_GET[action] == 'reply'}--><span>发表回复</span>
					...
					<!--{if $_G['uid']}-->
						...
						<!--{if $_GET[action] == 'newthread' && !$modnewthreads || $_GET[action] == 'edit' && $isfirstpost && !$modnewreplies && $thread['displayorder'] == -4}-->
							<button type="button" id="postsubmit" class="tt_big_fatie_btn" value="true" onclick="$('postsave').value = 1;$('postsubmit').click();"><em>{lang save_draft}</em></button>
						<!--{/if}-->
					<!--{/if}-->
					<!--{hook/post_btn_extra}-->
	...
	<!--{subtemplate forum/post_editor_tt}-->
</div>
</form>
