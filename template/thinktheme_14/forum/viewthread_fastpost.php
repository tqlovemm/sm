...
<div class="tt_f_pst_title">¿ìËÙ»Ø¸´</div>
...
<form method="post" autocomplete="off" id="fastpostform" action="forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]&extra=$_GET[extra]&replysubmit=yes{if $_GET['ordertype'] != 1}&infloat=yes&handlekey=fastpost{/if}{if $_GET[from]}&from=$_GET[from]{/if}"{if $_GET['ordertype'] != 1} onSubmit="return fastpostvalidate(this)"{/if}>
	...
	<div class="cl">
		<!--{if empty($_GET[from]) && $_G[setting][fastsmilies]}--><!--{/if}-->
...
<script type="text/javascript" src="{$_G[siteurl]}static/js/swfupload.js?{VERHASH}"></script>
<script type="text/javascript" src="{$_G[siteurl]}static/js/swfupload.queue.js?{VERHASH}"></script>
<script type="text/javascript" src="{$_G[siteurl]}static/js/handlers.js?{VERHASH}"></script>
<script type="text/javascript" src="{$_G[siteurl]}static/js/fileprogress.js?{VERHASH}"></script>
	...
	<p class="ptm pnpost">
		...
		<button {if $allowpostreply}type="submit" {elseif !$_G['uid']}type="button" onclick="showWindow('login', 'member.php?mod=logging&action=login&guestmessage=yes')" {/if}name="replysubmit" id="fastpostsubmit" class="tt_red_btn" value="replysubmit" tabindex="5"><strong>{lang post_newreply}</strong></button>&nbsp;&nbsp;&nbsp;
		<!--{hook/viewthread_fastpost_btn_extra}-->
		<!--{if helper_access::check_module('follow')}-->
		<label class="lb tt_f_pst_label"><input type="checkbox" name="adddynamic" class="pc" value="1" />{lang post_reply_relay}</label>
		<!--{/if}-->
		<!--{if $_GET['ordertype'] != 1 && empty($_GET[from])}-->
		<label class="tt_f_pst_label" for="fastpostrefresh"><input id="fastpostrefresh" type="checkbox" class="pc" />{lang post_fastreply_gotolast}</label>
