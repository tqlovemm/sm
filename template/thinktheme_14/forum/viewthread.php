...
<!--{if $modmenu['thread'] || $modmenu['post']}--><!--{/if}-->
...
function addfollow(uid,dom){
	...
	jQuery.ajax({
		type:"GET",
		url:"home.php",
		data:{mod:'spacecp',ac:'follower',op:'add',fuid:uid},
		dataType:'JSON',
...
<div class="tt_mianbaoxie">您现在的位置：<a href="forum.php">论坛</a> $navigation > <a href="forum.php?mod=viewthread&tid=$_G[tid]">$_G[forum_thread][short_subject]</a></div>
...
<div id="ct" class="cl tt_tiezixiangqing_box">
	<div id="pgt" class="pgs tt_postlist_pager cl {if $modmenu['thread']} bbs{/if}">		
        $multipage        
		...
		<!--{else}-->
            ...
            <a id="newspecial" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang send_posts}">发表新帖</a><!--{/if}-->
		<!--{/if}-->
        
		<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
			<a id="post_reply" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}">回复本帖</a>
		<!--{/if}-->
			</div>
...
<!--{if $modmenu['thread']}-->
	<div id="modmenu" class="xi2 pbm tt_tiezi_guanli">
		<!--{eval $modopt=0;}-->
		<!--{if $_G['forum']['ismoderator']}-->
			<!--{if $_G['group']['allowdelpost']}--><!--{eval $modopt++}--><a href="javascript:;" onclick="modthreads(3, 'delete')">{lang modmenu_deletethread}</a><span class="pipe">|</span> <a href="forum.php?mod=thread&action=edit&fid=$_G[fid]&tid=$_G[tid]" >编辑</a><span class="pipe">|</span><!--{/if}-->
...
<div id="postlist" class="tt_postlist">
	<table cellspacing="0" cellpadding="0" class="tt_postlist_top">
		<tr>
			<td class="tt_postlist_left">
				<!--{if $_G['page'] > 1}-->
				...
				<!--{else}-->
				<div class="hm">
					<span>{lang show}:</span> <b class="xi1">$_G[forum_thread][views]</b><span class="pipe">|</span><span>{lang reply}:</span> <b class="xi1">$_G[forum_thread][replies]</b>
				</div>
				<!--{/if}-->
			</td>
			<td class="tt_postlist_right">
				...
				<span class="xg1">
					...
					<!--{/if}-->
					<a href="thread-$_G[tid]-1-1.html{if $_G[uid]}?fromuid=$_G[uid]{/if}" onclick="return copyThreadUrl(this)" {if $fromuid}title="{lang share_url_copy_comment}"{/if}>[复制链接]</a>
				</span>
	...
	<table cellspacing="0" cellpadding="0" class="ad"><tr><td class=""></td><td class=""></td></tr></table>
	...
	<!--{loop $postlist $post}-->
			<!--{if $rushreply && $_GET['checkrush'] && $post['rewardfloor'] != 1}-->
				<!--{eval continue;}-->
			<!--{/if}-->
			<div id="post_$post[pid]" class="tt_postlist_box">
...
<!--{if $modmenu['thread']}-->
	<div class="xi2 mbm pbm bbs tt_tiezi_guanli">
	<script type="text/javascript">
...
<div class="pgs cl tt_postlist_pager">
...
<div class="tt_fatie_btn_box z">
	...
	<!--{if !$_G['forum_thread']['is_archived']}-->
		<a id="newspecialtmp" onmouseover="$('newspecial').id = 'newspecialtmp';this.id = 'newspecial';showMenu({'ctrlid':this.id})"{if !$_G['forum']['allowspecialonly'] && empty($_G['forum']['picstyle']) && !$_G['forum']['threadsorts']['required']} onclick="showWindow('newthread', 'forum.php?mod=post&action=newthread&fid=$_G[fid]')"{else} onclick="location.href='forum.php?mod=post&action=newthread&fid=$_G[fid]';return false;"{/if} href="javascript:;" title="{lang send_posts}">发表新帖</a>
	<!--{/if}-->
	<!--{if $allowpostreply && !$_G['forum_thread']['archiveid']}-->
		<a id="post_replytmp" onclick="showWindow('reply', 'forum.php?mod=post&action=reply&fid=$_G[fid]&tid=$_G[tid]')" href="javascript:;" title="{lang reply}">回复本帖</a>
	<!--{/if}-->
</div>    
...
<!--cnzz tui-->
	<script  type="text/javascript" charset="utf-8"  src="http://tui.cnzz.net/cs.php?id=1000047516"></script>
<!--cnzz tui-->
